...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE lib_master_tray set  
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO lib_master_tray(almirah_name, tray_name, max_quantity, created_by, creation_time) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from lib_master_tray where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from lib_master_tray where sno = '.$_GET['edit'];
						...
						<tr class="bg-primary text-white">
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td><a href="lib_master_tray.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="lib_master_tray.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
