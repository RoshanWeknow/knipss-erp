...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		...
		$sql = "delete from subject_fees where class_id=".$class_id;
		execute_query(connect(), $sql);
...
if(isset($_GET['del'])){
	$sql = 'delete from subject_fees where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
	
	$sql = 'delete from fees_detail where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
        	...
        	<div id="container" class="row d-flex my-auto">    	
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" class="wufoo leftLabel page1" name="addnewclass" enctype="multipart/form-data"
                 method="post" onSubmit="" >
    				<h2><img style="width:40px;" src="images/add.png" />Edit <span class="orange">Class</span></h2>
					<?php
                    switch($response){
                        case 1: {
                    ?>
						...
						<table width="100%" class="table table-striped table-hover rounded">
							<tr class="bg-primary text-white ">
								...
								while($row = mysqli_fetch_array($result)) {
									...
									<td>'.$row['category'].'</td>
									<td>'.$row['type'].'</td>
									<td><a href="editclass.php?id='.$row['sno'].'">Edit</a></td>
									<td><a href="editclass.php?del='.$row['sno'].'">Delete</a></td>
									</tr>';
								}  
								?>
						</table>
					...
					<?php
						...
						if(mysqli_num_rows($class_result)!=1){
							...
							$sql='select * from fees_detail where class_id="'.$class_row['sno'].'" and head_id="computer"';
							$computer=mysqli_fetch_array(execute_query(connect(), $sql));
							$sql='select * from fees_detail where class_id="'.$class_row['sno'].'" and head_id="self"';
							$self=mysqli_fetch_array(execute_query(connect(), $sql));
							$sql='select * from fees_detail where class_id="'.$class_row['sno'].'" and head_id="vocational"';
							$vocational=mysqli_fetch_array(execute_query(connect(), $sql));
							$sql='select * from fees_detail where class_id="'.$class_row['sno'].'" and head_id="tour"';
					...
					<div class="col-md-12">
						...
						<div class="row">							
							...
							<div class="col-md-4">							
								<label>Vocational <span class="orange">*</span></label>
								<input type="text" name="vocational" id="vocational" class="form-control" value="<?php if(isset($_POST['vocational'])){echo $_POST['vocational'];}else{echo $vocational['fee_amount'];} ?>"/>
							</div>
							<div class="col-md-4">							
					...
					<div class="card card-body" class="desc notranslate" for="s_class" style="display: none;" id="heads">
						<h3 >Heads</h3>
						<table class="table table-striped">
							...
							while($row = mysqli_fetch_array($res)) {
								$sql = "select * from `fees_detail` where class_id = '".$class_row['sno']."' and head_id = '".$row['sno']."'";
								...
								<td class="inp1"><div><input type="text" name="head'.$row['sno'].'" value="';
					...
					<div class="notranslate desc" style="display: none;" id="heads_self"><label  for="s_class">
						...
						<table class="table table-striped">
							...
							while($row = mysqli_fetch_array($res)) {
								...
								echo '<tr>
								<td class="inp1">'.$row['fee_type'].'</td>
								<td class="inp1"><div><input type="text" name="self_head'.$row['sno'].'" value="';
								if(isset($_POST['computer'])){
									echo $_POST['head'.$row['sno']];
								}
								else{
                    ...
                    while($row1 = mysqli_fetch_array($res1)) {
						$sql = 'select * from subject_fees where class_id='.$_GET['id'].' and subject_id='.$row1['sno'];
                        ...
                        echo '			
							<div>
								<input type="checkbox" name="check_'.$row1['sno'].'" '.$checked.' id="check_'.$row1['sno'].'" onchange="changeid(this.value)"  												                             	value="'.$row1['sno'].'"/>
							 		<li class="notranslate"><label  class="desc" for="ch_sub">('.$row1['sno'].')'.$row1['subject'].'</label>	
										<div id="show_'.$row1['sno'].'"';
											if($checked==''){
							...
							<table>
							<tr>
							<td class="inp1"><div><input type="text" name="code_'.$row1['sno'].'" value="'.$code.'"/></td>
							<td class="inp1"><div><input type="text" name="pract_'.$row1['sno'].'" value="'.$fees.'"/></td>
							<td class="inp1">
							<select name="type_'.$row1['sno'].'" id="type_'.$row1['sno'].'" class="inp1">
								<option ';
								if($type=='self'){
