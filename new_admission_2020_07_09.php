				...
				//echo '<b>Total Execution Time:</b> '.$execution_time1.' Mins';
				 ...
				 /*if($_POST['income_cert']!='' && $_POST['account_no']!='' && $_POST['opt_cat']=='SC'){
				  ...
				  if(isset($_POST['computer'])){
					...
					$sql='insert into fee_invoice (class_id, student_id, tot_amount, amount_paid, approval_date, e_pin, tc, marksheet, addmission_form, character_certificate, status, timestamp, user_id, type) values ("'.$class.'", "'.$stu_id['sno'].'", "'.$computer['fee_amount'].'", "'.$computer['fee_amount'].'", "'.$_POST['doa'].'", "'.$epin.'","1","1","1","1","1","'.strtotime($_POST['doa']).'","'.$_SESSION['username'].'","computer")';
					 execute_query(connect(), $sql); 
				 ...
				 if(isset($_POST['self'])){
					...
					$sql='insert into fee_invoice (class_id, student_id, tot_amount, amount_paid, approval_date, e_pin, tc, marksheet, addmission_form, character_certificate, status, timestamp, user_id, type) values("'.$class.'", "'.$stu_id['sno'].'", "'.$self['fee_amount'].'", "'.$self['fee_amount'].'", "'.$_POST['doa'].'", "'.$epin.'", "1", "1", "1", "1", "1", "'.strtotime($_POST['doa']).'", "'.$_SESSION['username'].'", "self")';
					 execute_query(connect(), $sql);
				 }
				 if(isset($_POST['tour'])){
					$sql='select * from fees_detail where class_id="'.$class.'" and head_id="tour"';
					$tour=mysqli_fetch_array(execute_query(connect(), $sql));
					$sql='insert into fee_invoice (class_id, student_id, tot_amount, amount_paid, approval_date, e_pin, tc, marksheet, addmission_form, character_certificate, status, timestamp, user_id, type) values("'.$class.'", "'.$stu_id['sno'].'", "'.$tour['fee_amount'].'", "'.$tour['fee_amount'].'", "'.$_POST['doa'].'", "'.$epin.'", "1", "1", "1", "1", "1", "'.strtotime($_POST['doa']).'", "'.$_SESSION['username'].'", "tour")';
					 execute_query(connect(), $sql); 
				 }
				 $sql = 'insert into fee_invoice (class_id, student_id, tot_amount, amount_paid, approval_date, e_pin, tc, marksheet, addmission_form, character_certificate, status, timestamp, user_id, type, mode_of_payment, chq_no) values("'.$class.'", "'.$stu_id['sno'].'", "'.$_POST['fees_amount'].'", "'.$_POST['fees_amount'].'", "'.$_POST['doa'].'", "'.$epin.'", "1", "1", "1", "1", "1", "'.strtotime($_POST['doa']).'", "'.$_SESSION['username'].'", "fees", "'.$_POST['mode_of_payment'].'", "'.$_POST['chq_no'].'")';
				 execute_query(connect(), $sql);
...
<body id="public">
    ...
    <h2 align="center">Application Form For <span class="orange">Admission (<?php echo $session_val['value']?>)</span></h2><hr />
           ...
           <li class="notranslate"><label  class="desc" for="m_name">Mother's Name <span class="alert">*</span></label>
                ...
                <div><input class="fieldtextmedium" id="" maxlength="15" size="20" name="fees"  value="<?php if(isset($_POST['fees'])){echo $_POST['fees'];} ?>" onFocus="fnTXTFocus(this.id)" >
            </div></li>
