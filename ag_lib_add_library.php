...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE ag_lib_add_library set 
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO ag_lib_add_library(library_name, created_by, creation_time) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from ag_lib_add_library where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from ag_lib_add_library where sno = '.$_GET['edit'];
						...
						<tr class="bg-primary text-white">
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td><a href="ag_lib_add_library.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="ag_lib_add_library.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
