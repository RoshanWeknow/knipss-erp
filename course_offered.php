...
<?php
	if(isset($_POST['name'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			...
			$sql = 'insert into course_offered (title, name, description, remarks, created_by, created_date, edited_by, edited_date) 
					values
				   ("'.$_POST['title'].'",
					"'.$_POST['name'].'",
					"'.$_POST['description'].'",
	...
	if(isset($_GET['del'])){
		$sql = 'delete from course_offered where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from course_offered where sno = '.$_GET['edit'];
						...
						<tr  class="bg-primary text-white" >
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td>'.$row['remarks'].'</td>
									<td><a href="course_offered.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="course_offered.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
