...
<html lang="en">
  ...
  <body class="w-100 m-auto" id="main">
        ...
        <div class="container-fluid" style="margin-top:5.2cm; ">
			
			<h3 class="text-center mb-4">STATEMENT OF MARKS<br><span style="font-size:16px;">2023-2024<br><br><span><h3>
            <table class="table table-borderless text-start" width="100%">
                ...
                <tr>
					...
					?>
					<?php //echo $photo; ?>
					<img style="width:80px; height:68px; " src="<?php echo $photo; ?>" alt="Student Image"></th>
				</tr>
				<tr>
				...
				<tr>
					<th class="look" colspan="4">University Enrollment No :</th>
						...
						if (!in_array($paperCode, $paperCodeArray)) {
							...
							<tr style="border:1px solid black; border-style:;">
								...
								?>	
									...
									<?php
									if($row4['theory_practical']=="Viva-voce"){
										echo '<td class="text-center">'.$practical_marks_max.'</td>
										<td class="text-center">'.$practical_marks_obt.'</td>';
									}
									else{
										$total_max_sub = ((float)$theory_marks_max+(float)$mid_marks_max);
										echo '<td class="text-center">'.$total_max_sub.'</td>';
									
										if ($grace_flag_print == 1 || $grace_flag_print == 2) {
											echo '<td class="text-center">'.$total_obt_sub . ' <sup>#</sup></td>';
										} else {
											if($total_obt_sub=='Abs' && $mid_marks_obt!='Abs' && $mid_marks_obt!='0' && $mid_marks_obt!=''){
												echo '<td class="text-center">'.$mid_marks_obt.'</td>';
											}
											else{
												echo '<td class="text-center">'.$total_obt_sub.'</td>';
											}
										}
									}
										?>
									<td class="text-center"><?php echo $credit_paper_t; ?></td>
									<td class="text-center"><?php echo $earned_credit_t; ?></td>
									<td class="text-center"><?php echo $grade_erned_t; ?></td>
									<td class="text-center"><?php echo $grade_credit_erned_t; ?></td>
							...
							<tr style="border:1px solid black; border-style:;">
								...
								<td><?php echo $paperCode; ?></td>
								<td><?php echo $row4['title_of_paper']; ?></td>
								<td class="text-center"><?php echo ((float)$theory_marks_max+(float)$mid_marks_max) ?></td>				
								<td class="text-center"><?php $total_obt_t = ((float)$theory_marks_obt+(float)$mid_marks_obt); 
									if ($grace_flag_print == 1 || $grace_flag_print == 2) {
							...
							<?php
								...
								<tr style="border:1px solid black; border-style:;">
									...
									?>
										...
										<td style="position:relative;" ><div class="merge_column"><?php echo $row5['title_of_paper']; ?></div></td>
										<td style="position:relative;" class="text-center"><div class="merge_column"><?php echo $practical_marks_max_p_t = $practical_marks_max_p+$total_max_sub; ?></td>
										<td style="position:relative;" class="text-center"><div class="merge_column">
										<?php 
										...
										<td style="position:relative;" class="text-center"><div class="merge_column"><?php echo $credit_paper_p_t = $credit_paper_p+$credit_paper_t; ?></div></td>
										...
										if($practical_marks_obt_p_show == 'Abs'){
										 ...
										 ?></div></td>
										<td style="position:relative; border:none;" class="text-center"><div class="merge_column"><?php echo $grade_erned_p_t = calculate_grade($practical_marks_obt_p_t, $practical_marks_max_p_t);?></div></td>
										<td style="position:relative;" class="text-center"><div class="merge_column"><?php echo $grade_credit_erned_p_t = ((float)$earned_credit_p_t*(float)$grade_erned_p_t); ?></div></td>
										<td style="position:relative;" class="text-center"><div class="merge_column"><?php echo (($practical_marks_obt_p_show=='Abs')?'AB':calculate_grade_letter($practical_marks_obt_p_t, $practical_marks_max_p_t)).'</td>';?></div></td>
									...
									?>
										...
										<td class="text-center"><?php echo $practical_marks_max_p; ?></td>
										<td class="text-center"><?php echo $practical_marks_obt_p_show; ?></td>
										<td class="text-center"><?php echo $credit_paper_p; ?></td>
										<td class="text-center"><?php echo $earned_credit_p; ?></td>
										<td class="text-center"><?php echo $grade_erned_p; ?></td>
										<td class="text-center"><?php echo $grade_credit_erned_p; ?></td>
										<td class="text-center"><?php
												if($practical_marks_obt_p_show=="Abs"){
												 echo 'AB';	
												}else{
			...
			<table width="100%" class="table  text-center" style="border:1px solid black; ">
				...
				<?php
					...
					if (mysqli_num_rows($result_sem_even) != 0) {
						...
						$sub_html .= '<tr style="border:1px solid black; border-style:;">
								...
								<td><b>' . $row_sem_even['passing_status'] . '</b></td>
							</tr>';
				...
				<tr>
					<td><center><?php 
							$sql_class = 'select * from class_detail where sno = "'.$row['course_name'].'"';
							$row_class = mysqli_fetch_assoc(mysqli_query($db,$sql_class));
							echo $row_class['class_description'];  ?></center></td>
					...
					<td><center><?php echo number_format($tot_tot_cgpa, 2); ?></center></td>
					<td><center><b><?php if (!empty($incpaperArray)) {
						echo 'INC';
						} elseif ($passing_status == 'PASSED' || $passing_status == 'ATKT') {
							if ($grace_flag != 0) {
								echo $passing_status . ' WITH GRACE';
							} else {
								echo $passing_status ;
							}
						} else {
				...
				<!--<table width="100%"  >
				<tr>
                    <th colspan="3" class="text-center p-2">DETAILS OF BACKLOG PAPER</th>
                </tr>
				<tr>
				...
				<tr>
					<td><center><?php 
							// $sql_class = 'select * from class_detail where sno = "'.$row['course_name'].'"';
							// $row_class = mysqli_fetch_assoc(mysqli_query($db,$sql_class));
							// echo $row_class['class_description'];  ?></center>
					</td>
					<td><center><?php 
				...
				<tr>
					<td colspan="3">NC = NON CREDITED , NA = NOT APPLICABLE , ATKT = ALLOWED TO KEEP TERM , # = PASSED WITH GRACE <br> Equivalent Percentage of Marks=CGPA*10</td>
				</tr>
				</table>-->
			
			

 <p style="text-align:left;"><b>Equivalent Percentage of Marks=CGPA*10 <br>NC = NON CREDITED , NA = NOT APPLICABLE , ATKT = ALLOWED TO KEEP TERM <br> # = PASSED WITH GRACE <br> </b></p>
<table width="100%">
			
    <tr>
        <th width="30%">
            <img src="image/barcode.gif" style="height:40px;"><br>RESULT DECLARATION DATE : 30/01/2024 
        </th>
        <th  width="40%" class="text-center">
            <img src="image/KNI20233658.png" style="height:70px;">
        </th>
		<th  width="30%">    </th>
    </tr>
    <tr>
        <th style="margin-top:15px;font-size:16px!important;" valign="bottom">Principal </th>
        <th class="text-center"> Checked By : <br><br>1. <br><br><br>2. </th>
        <th style="font-size:16px!important; text-align:right;" valign="bottom"> Controller of Examination </th>
    </tr>
</table>
