...
if(isset($_GET['dis'])){
	$sql = 'select * from exam_student_info where sno="'.$_GET['dis'].'"';
	//echo $sql;
	$row = mysqli_fetch_assoc(mysqli_query($db, $sql));
	
	if($row['admit_card_allow']=='1'){
		$sql = 'update exam_student_info set admit_card_allow="0" where sno="'.$row['sno'].'"';
	}
	else{
		$sql = 'update exam_student_info set admit_card_allow="1" where sno="'.$row['sno'].'"';
	}
	//echo $sql;
	mysqli_query($db, $sql);
	//$sql = 'update exam_student_info set admit_card_allow="'.
}
...
<div id="container">
		<div class="card card-body">
			...
			<div class="row">
					...
					<div class="row">
						<div class="col-md-6">
							...
							<select name="course_name" id="course_name" class="form-control" onChange="fn_year_part(this.value, '<?php echo $_POST['year_part']; ?>');">
								<option value=""></option>
								<?php
								$sql = 'select * from class_detail where 1=1 order by abs(display_sort)';
				...
				if(isset($_POST['course_name'])){
				?>
				<form action="exam_marksheet_pdf_print.php" method="GET" enctype="multipart/form-data" id="" name="" target="_blank">
				<div class="row">
				<div class="col-md-4">
