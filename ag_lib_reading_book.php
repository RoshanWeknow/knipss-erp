...
<?php
	if(isset($_POST['submit2'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			...
			$query = 'select * from ag_lib_reading_book where accession_no = "'.$_POST['accession_no'].'" AND status =1';
			$res = execute_query($db,$query);
			if(mysqli_num_rows($res)==0){
				$sql = 'insert into ag_lib_reading_book (lib_card_no, stu_name, mobile, accession_no, subject, author_name, booksissuedate, created_by, creation_time)
				VALUES ("'.$_POST['lib_card_no'].'","'.$_POST['stu_name'].'","'.$_POST['p_mobile'].'","'.$_POST['accession_no'].'", "'.$_POST['subject_heading'].'", "'.$_POST['author_name'].'", "'.$_POST['booksissuedate'].'", "'.$_SESSION['username'].'","'.date('d-m-y H:m:s').'")';
				//echo $sql;
				execute_query($db,$sql);
				if(mysqli_errno($db)){
	...
	if(isset($_GET['del'])){
		$sql = 'delete from ag_lib_reading_book where sno="'.$_GET['del'].'"';
		execute_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from ag_lib_reading_book where sno = '.$_GET['edit'];
		...
		<div class="card row" style="--bs-gutter-x: -0.5rem !important;">
			<div class="col-md-12">
				<div class="issue-wrapper">
					...
					<div class="bg-primary text-white p-2"><h3>Reading Book</h3></div>
						...
						if (isset($_POST["submit1"])) {
						   $sql  = 'select * from ag_lib_id_card where lib_no = "'.$_POST['roll_no'].'"';
						   //echo $sql;
						...
						<table width="100%" class="table table-striped table-hover rounded">
							<tr>
								...
								<td>
								   <input type="text" class="form-control" name="lib_card_no" value="<?php echo $lib_no; ?>" readonly> 
								</td>
