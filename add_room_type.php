...
if (isset($_POST['name'])) {
	...
	if ($msg == '') {
		if ($_POST['edit'] != '') {
			$sql = 'update add_room_type set 
			name="' . $_POST['name'] . '",
			edited_by="' . $_SESSION['username'] . '",
			edition_time="' . date("Y-m-d H:i:s") . '"
			where sno=' . $_POST['edit'];
		} else {
			$sql = 'insert into add_room_type (name, created_by, creation_time) values ("' . $_POST['name'] . '", "' . $_SESSION['username'] . '", "' . date("Y-m-d H:i:s") . '")';
		}
...
if (isset($_GET['id'])) {
	$sql = 'select * from add_room_type where sno=' . $_GET['id'];
	$data = mysqli_fetch_assoc(execute_query($db, $sql));
	$_POST['name'] = $data['name'];
	$_POST['edit'] = $data['sno'];
}
if (isset($_GET['del'])) {
		$sql = 'delete from add_room_type where sno=' . $_GET['del'];
				...
				<table class="table table-hover table-striped">
					...
					<?php
									...
									while($row = mysqli_fetch_assoc($result)){
										...
										<td>'.$i++.'</td>
										<td>'.$row['name'].'</td>
										<td><a href="add_room_type.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
										<td><a href="add_room_type.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
											</tr>'	;
									}
								?>
