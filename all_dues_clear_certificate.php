...
if(isset($_POST['enrollment_no']) && $_POST['enrollment_no'] != ''){
	...
	if(isset($_POST['edit']) && $_POST['edit']!= ''){
		$sql = 'update ct_all_due_clear_certificate set 
		...
		$sql = 'insert into ct_all_due_clear_certificate(enrollment_no,name,institution_name,course_name,issue_date,description,created_by,creation_time) values(
		"'.$_POST['enrollment_no'].'","'.
		$_POST['name'].'","'.
...
if (isset($_GET['edit'])) {
	$sql = 'select * from ct_all_due_clear_certificate where sno=' . $_GET['edit'];
	$data = mysqli_fetch_assoc(execute_query($db,$sql));
}

if(isset($_GET['del']) and $_GET['del']!='') {
		$sql = 'delete from ct_all_due_clear_certificate where sno=' . $_GET['del'];
		...
		<div class="card card-body">    
        	<div class="row d-flex my-auto">  
				<form action="<?php echo $_SERVER['PHP_SELF'] ?>" class="wufoo leftLabel page1" name="feesdeposit" enctype="multipart/form-data" method="post" onSubmit="" autocomplete="off" >
					<h3> All Due Clear Certificate</h3>
						<div class="col-md-12" >
							<!-- first row -->
							<div class="row">							
								<div class=" col-md-3 ">							
		...
		<div class="card card-body">
			<table  class="table table-striped table-hover rounded">
				...
				<?php
					...
					$sql = 'select * from ct_all_due_clear_certificate';
