...
<?php
	if(isset($_POST['submit'])){
		$ins_sql = 'INSERT INTO `mid_sem_marks_allotment`(`paper`, `examiner`, `v_auth`, `max_marks`, `stu_count`, `allot_stu`, `created_by`, `creation_time`) VALUES("'.$_POST['paper'].'",
							"'.$_POST['examiner'].'",
							"'.$_POST['v_auth'].'",
							"'.$_POST['max_marks'].'",
							"'.$_POST['stu_count'].'",
		...
		$mid_allot_sno = mysqli_insert_id($db);
		//print_r($_POST);
		...
		for($a=1;$a<=$_POST['allot_stu'];$a++){
			...
			if($_POST['exam_stu_type'.$a]=='Regular'){
    			...
    			mid_sem_marks_max='" .$_POST['max_marks']."',
    			mid_sem_marks_obt='" .$_POST['obt_marks'.$a]."',
    			mid_teacher='" .$_POST['examiner']."',
    			mid_verifier='" .$_POST['v_auth']."',
    			mid_sem_allotment='$mid_allot_sno'
		...
		<div class="row">
            <div class="col-md-12">
                <div class="card">
					...
					<?php echo $msg; ?> 
                    <div class="col-md-12">
						<h2 class="bg-primary text-white p-2">Mid Sem Marks Feed</h2>
				        <!-- first row -->
                        <div class="row">
							 <div class=" col-md-4 ">
					...
					if(isset($_POST['save']) || isset($_POST['search'])){
						...
						//$sql = 'select count(*) stu_c from ((SELECT sno FROM `back_exam_student_paper_info` where paper_code = "'.$_POST['paper'].'" AND mid_sem_allotment = "0") union all (select sno from exam_student_paper_info where paper_code = "'.$_POST['paper'].'" AND mid_sem_allotment = "0" )) t2';
						$sql = 'SELECT 
								COUNT(*) AS stu_c,
								t2.sno AS id,
								t2.exam_roll_no,
							...
							FROM (
								...
								WHERE 
									back_exam_student_paper_info.paper_code = "' . $_POST['paper'] . '" 
									AND back_exam_student_paper_info.mid_sem_allotment = "0"
									AND back_exam_student_info.exam_roll_no IS NOT NULL
								...
								WHERE 
									exam_student_paper_info.paper_code = "' . $_POST['paper'] . '" 
									AND exam_student_paper_info.mid_sem_allotment = "0"
									AND exam_student_info.exam_roll_no IS NOT NULL
							) AS t2
							ORDER BY 
								t2.exam_roll_no';
					...
					<button type="submit" name = "search" value="search" class="btn btn-primary mt-2 ms-2">SEARCH</button> 
						<a href="exam_mid_sem_marks_allotment_report.php" target="_blank">View Report</a>
			...
			<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data" id="" onsubmit="return confirm('Are you sure?');">	
				...
				<?php
					...
					for($i=1;$i<=$_POST['ccount'];$i++){
						$stu_allot = 'SELECT *,exam_student_paper_info.sno as id FROM `exam_student_paper_info` LEFT JOIN exam_student_info on exam_student_info_sno=exam_student_info.sno WHERE paper_code="'.$_POST['paper'].'" AND exam_roll_no BETWEEN "'.$_POST['fromroll'.$i].'" AND "'.$_POST['toroll'.$i].'" AND mid_sem_allotment="0" ORDER BY exam_roll_no';
						
						//$stu_allot = '(SELECT exam_student_paper_info.sno as id, student_name, "regular" as type FROM `exam_student_paper_info` LEFT JOIN exam_student_info on exam_student_info_sno=exam_student_info.sno  WHERE paper_code="'.$_POST['paper'].'" AND exam_roll_no BETWEEN "'.$_POST['fromroll'.$i].'" AND "'.$_POST['toroll'.$i].'" AND mid_sem_allotment="0") union all (SELECT back_exam_student_paper_info.sno as id, student_name, "back" as type FROM `back_exam_student_paper_info` LEFT JOIN back_exam_student_info on exam_student_info_sno=back_exam_student_info.sno  WHERE paper_code="'.$_POST['paper'].'" AND exam_roll_no BETWEEN "'.$_POST['fromroll'.$i].'" AND "'.$_POST['toroll'.$i].'" AND mid_sem_allotment="0")';
						$stu_allot = 'SELECT 
									...
									WHERE 
										...
										AND exam_roll_no BETWEEN "'.$_POST['fromroll'.$i].'" AND "'.$_POST['toroll'.$i].'" AND mid_sem_allotment="0"
