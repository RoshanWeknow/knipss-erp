					...
					<table class="table table-striped table-hover" id="general_stat_table">
						<thead>
							<tr class="bg-primary text-white">
								...
								<td scope="col">Sno</td>
								<td scope="col">Mid-Term</td>
								<td scope="col">Theory</td>
						...
						while($row_subject_details = mysqli_fetch_assoc($result_paper)){
							if($row_subject_details['type_status']=='1'){
								$_POST['course'] = $row_subject_details['class_id'];
								$_POST['paper'] = $row_subject_details['sno'];
								$_POST['subject'] = $row_subject_details['subject_id'];
								...
								if(isset($_POST['course'])){
									...
									$query = '(SELECT exam_student_info.sno as exam_sno, student_info.sno as sno, student_name, student_info_sno, stu_name, exam_form_no, exam_roll_no, uin_no, course_name FROM `exam_student_info` left join student_info on student_info.sno = student_info_sno  WHERE course_name="'.$_POST['course'].'" and (sub1="'.$_POST['subject'].'" or sub2="'.$_POST['subject'].'" or sub3="'.$_POST['subject'].'") and exam_roll_no is not null and exam_roll_no!="" and student_info.sno not in (select student_id from student_info2 where type="subject_change"))
									union all
									(SELECT exam_student_info.sno as exam_sno, student_info2.student_id as sno, student_name, student_info_sno, stu_name, exam_form_no, exam_roll_no, uin_no, course_name FROM `exam_student_info` left join student_info2 on student_info2.student_id = student_info_sno  WHERE course_name="'.$_POST['course'].'" and (sub1="'.$_POST['subject'].'" or sub2="'.$_POST['subject'].'" or sub3="'.$_POST['subject'].'") and exam_roll_no is not null and exam_roll_no!="" and student_info2.type="subject_change")
									...
									if($_POST['course']=='112'){
										...
										union all
										(SELECT exam_student_info.sno as exam_sno, student_info2.student_id as sno, student_name, student_info_sno, stu_name, exam_form_no, exam_roll_no, uin_no, course_name FROM `exam_student_info` left join student_info2 on student_info2.student_id = student_info_sno  WHERE course_name="'.$_POST['course'].'" and sub1 in (37, 66, 67, 68) and exam_roll_no is not null and exam_roll_no!="" and student_info2.type="subject_change") order by exam_roll_no';
									}
								...
								while($row=mysqli_fetch_assoc($result)){
									...
									$sql = 'select * from exam_student_paper_info where exam_student_info_sno="'.$row['exam_sno'].'" and paper_code="'.$row_subject_details['paper_code'].'"';
									//echo $sql;
									$result_selection = mysqli_query($db, $sql);
									if(mysqli_num_rows($result_selection)){
                                        $row_paper_info = mysqli_fetch_assoc($result_selection);
										...
										?>
											<tr>
												...
												<td><?php echo $row_stu_info['gender'] ;?></td>
												<td><?php echo $subject; ?></td>
												<td><?php echo $paper; ?></td>
												<td><?php echo $row_paper_info['sno']; ?></td>
												<td><?php echo $row_paper_info['mid_sem_marks_obt']; ?></td>
												<td><?php echo $row_paper_info['pt_marks_obt']; ?></td>
								...
								$_POST['course'] = $row_subject_details['class_id'];
								$_POST['paper'] = $row_subject_details['sno'];
								$_POST['other_subject'] = $row_subject_details['subject_id'];
								...
								if(isset($_POST['course'])){
									$query = 'SELECT student_info.sno as sno, exam_student_info.sno as exam_sno, student_name, student_info_sno, stu_name, exam_form_no, exam_roll_no, uin_no, course_name FROM `exam_student_info` left join student_info on student_info.sno = student_info_sno  WHERE course_name ="'.$_POST['course'].'" and exam_roll_no is not null and exam_roll_no!="" order by exam_roll_no';
									...
									$query = 'SELECT *, exam_student_info.sno as exam_sno FROM exam_student_info';
								}
								//echo $query.'<br>';
								$result =execute_query($db,$query);
								while($row=mysqli_fetch_assoc($result)){
