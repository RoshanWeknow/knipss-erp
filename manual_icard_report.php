...
$sql="DELETE FROM `student_info_mannul_icard` WHERE sno='".$_GET['del']."'";
$run=execute_query(connect(), $sql);
	...
	$msg .= '		<div class="card card-body">
		...
		$sql="select * from `student_info_mannul_icard` where 1=1";
		 $condition=array();
            ...
            if($_POST['roll_no'] !="") {
             ...
             while($row=mysqli_fetch_array($run)){
             	$msg .= '<tr>
						...
						<td>'.$row['roll_no'].'</td>
						<td>'.$row['e_mail1'].'</td>
						<td><a href="manual_icard.php?id='.$row['sno'].'">Edit</td>
						<td><a href="manual_icard_report.php?del='.$row['sno'].'">Delete</td>
						<td><a href="manual_icard_print.php?id='.$row['sno'].'" target="_blank">Print</td>
						</tr>';
						$i++;
