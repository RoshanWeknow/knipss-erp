...
if (isset($_POST['name'])) {
	...
	if ($msg == '') {
		if ($_POST['edit'] != '') {
			$sql = 'update add_building_condition set 
			name="' . $_POST['name'] . '",
			edited_by="' . $_SESSION['username'] . '",
			edition_time="' . date("Y-m-d H:i:s") . '"
			where sno=' . $_POST['edit'];
		} else {
			$sql = 'insert into add_building_condition (name, created_by, creation_time) values ("' . $_POST['name'] . '", "' . $_SESSION['username'] . '", "' . date("Y-m-d H:i:s") . '")';
		}
...
if (isset($_GET['id'])) {
	$sql = 'select * from add_building_condition where sno=' . $_GET['id'];
	$data = mysqli_fetch_assoc(execute_query($db, $sql));
	$_POST['name'] = $data['name'];
	$_POST['edit'] = $data['sno'];
}
if (isset($_GET['del'])) {
		$sql = 'delete from add_building_condition where sno=' . $_GET['del'];
			...
			<div class="card-body table-full-width table-responsive">
				<table class="table table-hover table-striped">
					<thead>
						<?php
						$i = 1;
						$sql = 'select * from add_building_condition where 1=1';
