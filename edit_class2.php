...
if(isset($_POST['submit'])){
	...
	$sql = 'delete from fees_detail2 where class_id="'.$class_id.'"';
	...
	if($_POST['class_type']!='SELF'){
		...
		$sql_fees='insert into fees_detail2(class_id, head_id,fee_amount) value
	...
	if($msg==''){
		...
		execute_query($link, $sql_fees);
		//echo $sql_fees;
		if(mysqli_error($link)){
			echo mysqli_error().'<br>'.$sql.'<br>';
		}
			
		$sql = "delete from subject_fees2 where class_id=".$class_id;
		execute_query($link, $sql);
		...
		$sql='insert into subject_fees2(class_id,subject_id,fees,type,code) values';
...
if(isset($_GET['del'])){
	$sql = 'delete from subject_fees2 where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
	
	$sql = 'delete from fees_detail2 where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
...
<body id="public">
        	...
        	<div id="container">    	
				<form action="edit_class2.php" class="wufoo leftLabel page1" name="addnewclass" enctype="multipart/form-data"
                 method="post" onSubmit="" >
    				<h2><img style="width:40px;" src="images/add.png" />Edit <span class="orange">Class (SI GEN/OBC)</span></h2>
					<?php
                    switch($response){
                        case 1: {
                    ?>
				...
				<div class="card card-body">
                    ...
                    while($row = mysqli_fetch_array($result)) {
					  ...
					  if($z%2!=0){
						...
						<td>'.$row['category'].'</td>
						<td>'.$row['type'].'</td>
						<td><a href="edit_class2.php?id='.$row['sno'].'">Edit</a></td>
						<td><a href="edit_class2.php?del='.$row['sno'].'">Delete</a></td>
						</tr>';
					}  
					?>
					</table>
					...
					<?php
						...
						if(mysqli_num_rows($class_result)!=1){
							...
							$sql='select * from fees_detail2 where class_id="'.$class_row['sno'].'" and head_id="computer"';
							$computer=mysqli_fetch_array(execute_query(connect(), $sql));
							$sql='select * from fees_detail2 where class_id="'.$class_row['sno'].'" and head_id="self"';
							$self=mysqli_fetch_array(execute_query(connect(), $sql));
							$sql='select * from fees_detail2 where class_id="'.$class_row['sno'].'" and head_id="tour"';
   					...
   					<div class="card card-body" style="display: none;" id="heads">
						...
						<table width="100%" class="table table-striped table-hover rounded">
							...
							while($row = mysqli_fetch_array($res)) {
								$sql = "select * from `fees_detail2` where class_id = '".$class_row['sno']."' and head_id = '".$row['sno']."'";
					...
					<div class="card card-body desc"   for="sub">
						...
						<fieldset style="width:950px;" class="table table-striped table-hover rounded">	
							...
							while($row1 = mysqli_fetch_array($res1)) {
								$sql = 'select * from subject_fees2 where class_id='.$_GET['id'].' and subject_id='.$row1['sno'];
								...
								echo '			
									<div class="col-md-12  border rounded m-1 p-2">
										<div class="row">
											...
											<div class=" class="col-md-3"">
												<label  class="desc" for="ch_sub">('.$row1['sno'].')'.$row1['subject'].'</label>
											</div>
									...
									<table width="100%" class="table table-striped table-hover rounded">
										<tr class=" ">
											...
											<td class="inp1">
												<select name="type_'.$row1['sno'].'" id="type_'.$row1['sno'].'" class="form-control">
													<option ';
													if($type=='self'){
