...
if(isset($_POST['enrollment_no']) && $_POST['enrollment_no'] != ''){
	...
	if(isset($_POST['edit']) && $_POST['edit']!= ''){
		$sql = 'update ct_literature_certificate set 
			enrollment_no="' . $_POST['enrollment_no'] . '",
			name="' . $_POST['name'] . '",
			institution_name="' . $_POST['institution_name'] . '",
			course_name="' . $_POST['course_name'] . '",
			course_starting_date="' . $_POST['course_starting_date'] . '",
			course_ending_date="' . $_POST['course_ending_date'] . '",
		...
		$sql = 'insert into ct_literature_certificate(enrollment_no,name,institution_name,course_name,course_starting_date,course_ending_date,issue_date,description,created_by,creation_time) values(
		"'.$_POST['enrollment_no'].'","'.
		$_POST['name'].'","'.
		$_POST['institution_name'].'","'.
		$_POST['course_name'].'","'.
		$_POST['course_starting_date'].'","'.
		$_POST['course_ending_date'].'","'.
		$_POST['issue_date'].'","'.
		$_POST['description'].'","'.
		$_SESSION['username'].'","'.
		date('Y-m-d H:m:s').'")';
...
if (isset($_GET['edit'])) {
	$sql = 'select * from ct_literature_certificate where sno=' . $_GET['edit'];
	$data = mysqli_fetch_assoc(execute_query($db,$sql));
}

if(isset($_GET['del']) and $_GET['del']!='') {
		$sql = 'delete from ct_literature_certificate where sno=' . $_GET['del'];
							...
							<div class="row">
								<div class="col-md-3 ">
									<div class="form-group">
										...
										<script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $data['course_starting_date']: date('Y-m-d');
												echo "document.writeln(DateInput('course_starting_date', 'user_form', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script>
									</div>
								</div>
								<div class="col-md-3 ">
									<div class="form-group">
										...
										<script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $data['course_ending_date']: date('Y-m-d');
												echo "document.writeln(DateInput('course_ending_date', 'user_form', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script>
									</div>
								</div>
		...
		<div class="card card-body">
			<table  class="table table-striped table-hover rounded">
				...
				<?php
					...
					$sql = 'select * from ct_literature_certificate';
				...
				<tr>
					...
					<td><?php echo $row['course_starting_date'] ?></td>
					<td><?php echo $row['course_ending_date'] ?></td>
