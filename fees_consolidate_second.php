...
if(isset($_POST['submit1'])){
	$date=$_POST['dfcdate'];
	$sql = 'select * from fees_transfer2 where month="'.date("Y-m", strtotime($date)).'" and type="aided" and head_id not in ("transfer", "deduction", "maintenance")';
...
function report_print_class() {
	window.open("fees_consolidate_second_print.php");
}
</script>
<body id="public">
	<div id="container">	
		<div class="card card-body">    
        	<div class="row d-flex my-auto">    	
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="wufoo leftLabel page1" name="feesdeposit" enctype="multipart/form-data" method="post" onSubmit="" >
					<h2>Fees <span class="orange">Consolidate (Class Wise) Second Installment</span></h2>
					<div class="col-md-12">
						<div class="row">							
							<div class="col-md-4">							
							...
							<div class="col-md-4">							
								...
								<select class="form-control" name="type" id="type">
									...
									<option value="self" <?php if(isset($_POST['type'])){if($_POST['type']=='self'){echo ' selected';}}?>>SELF FINANCE</option>
									<option value="aided" <?php if(isset($_POST['type'])){if($_POST['type']=='aided'){echo ' selected';}}?>>AIDED</option>
									<option value="ballb" <?php if(isset($_POST['type'])){if($_POST['type']=='ballb'){echo ' selected';}}?>>BA.LLB.</option>
								 </select>
							</div>
						</div>
						...
						<input type="button" name="student_ledger1" class="btn btn-danger" oonClick="return report_print_class()" style="float: right;" value="Print">					
				 </form>
			</div>
		</div> 
...
?>
		...
		if(isset($_POST['dfc_date'])){
			...
			$start = date("Y-m-01", strtotime($_POST['dfc_date']));
			$end = date("Y-m-d", strtotime($start."+1 month"));
		}
		$_SESSION['start5']=$start;
		$_SESSION['end15']=$end;
		$_SESSION['fees_date5']=$_POST['dfc_date'];
		$i=1;
		...
		$res=execute_query(connect(), $sql);
			...
			$ballb = '';
			$fee_type='';
				$timestamp= " and approval_date>='".$start."' and approval_date<'".$end."' and fee_invoice3.class_id='".$class['sno']."'";
				if(isset($_POST['type'])){
				if($_POST['type']=='self'){
				...
				$_SESSION['class_type5']=$_POST['type'];
			}
		...
		<td>'.$tot_stu_count.'</td>';
			...
			echo '<td>'.$grand_tot[30].'<input type="hidden" name="fees_total_31" value="'.$grand_tot[30].'"></td>';
			...
			echo '<input type="Submit" name="submit1" value="Generate Fees Collection" class="submit">
			</form>';
