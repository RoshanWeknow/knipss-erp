						...
						<div class="row">
							 <div class=" col-md-4 ">
								...
								<select name="corsslist_course_llb" id="course" value="" class="form-control" tabindex="<?php echo $tabindex++;?>" required>
									...
									<?php 
									$sql  = 'select distinct(course_name),class_detail.class_description from exam_student_info LEFT JOIN class_detail on exam_student_info.course_name = class_detail.sno WHERE class_detail.crasslist_type = 3 ORDER BY class_detail.class_description';
									//echo $sql;
...
case 1:{
	if(isset($_POST['corsslist_course_llb'])){
		$sql = 'SELECT `exam_student_info`.`sno` as id, exam_student_info.student_info_sno ,`exam_student_info`.`student_name`,`exam_student_info`.`exam_roll_no`,`exam_student_info`.`dob`,`exam_student_info`.`exam_id`,`exam_student_info`.`uin_no`,`exam_student_info`.`course_name`,`student_info`.`father_name`, `student_info`.`gender` FROM `exam_student_info` LEFT JOIN `student_info` ON `exam_student_info`.student_info_sno = `student_info`.sno where exam_roll_no!="" and exam_roll_no is not null and `exam_student_info`.`course_name` = "'.$_POST['corsslist_course_llb'].'" order by exam_roll_no';
	...
	<body>
		<div class="row noprint">
			<div class="col-md-12">
				...
				echo '<div class="pagination"><ul><li><a href="exam_crasslist_print_llb24.php?back=1">Back</a></li>';
				if ($total_pages > 1) {
			...
			<div class="wrap">
				<table width="100%">
					<tr style="border:1px solid black;border-style: ;">
						...
						<td width="6%" class="border border-dark"><b>TOTAL</br>600<br>270</b></td>
				...
				<div class="topmargin" style="">
					...
					<table width="100%">
						<tr style="border:1px solid black;border-style: ;">
							...
							<td width="15%" class="text-start ps-5" ><?php echo $row['student_name']; ?><b><?php //echo $row['dob']; ?></b><?php //echo $row['id']; ?></br><?php echo $row['father_name']; ?></td>
							<?php
								if(empty($papers)){
									$sql_paper_save = 'select * from exam_student_paper_info where exam_student_info_sno = "'.$row['id'].'"';
								...
								<td width="6%">
									<?php 
										if($row2['pt_marks_obt'] < 36){
											echo "<div style='text-align: center;'>".$row2['pt_marks_obt']."<br><span >--</span></div>";
										}
										else{
											echo $row2['pt_marks_obt']; 
										}
							...
							<?php
								...
								if($total_obt<270){
									$passing_status = "FAILED";
									$passing_status_reason = "Total OBT less then 270";
									//echo"000000";
								}
								if($total_abs == 0){
							...
							<td width="6%"><?php 
								if($total_obt == 0){
									echo "Abs";
								}elseif($total_obt < 270){
									echo "<div style='text-align: center;'>".$total_obt."<br><span >--</span></div>";
								}
								else{
									echo $total_obt; 
								}
							...
							<?php
								...
								$b_paper = NULL;
								$sql_update = 'UPDATE `exam_student_info` SET `max_marks`="'.$total_max.'",
								`obt_marks`="'.$total_obt.'",
								`passing_status`="'.$passing_status.'",
								`back_papers`="'.$b_paper.'" WHERE sno = '.$row['id'];
					...
					<table width="100%">
						<tr style="border:1px solid black;border-style: ;">
							...
							<?php
								...
								//$b_paper = implode(",", $backpaper);
								$b_paper = NULL;
								$sql_update = 'UPDATE `exam_student_info` SET `max_marks`="'.$total_max.'",`obt_marks`="'.$total_obt.'",`passing_status`="'.$passing_status.'",`back_papers`="'.$b_paper.'" WHERE sno = '.$row['id'];
								//$result_update = mysqli_query($db, $sql_update);
							?>
						</tr>
					</table>	
...
//******************************LLB 3rd Sem Start**********************//			
		...
		?>
			<div class="wrap">
				<table width="100%">
					<tr style="border:1px solid black;border-style: ;">
						<td width="8%" class="border border-dark"><b>ROLL NO</b></td>
						<td width="15%" class="text-start ps-5"><b>NAME OF THE CANDIDATE & </br> FATHER NAME</b></td>
						<td width="6%" class="border border-dark"><b>PAPER I<br>100</br>36</b></td>
						<td width="6%" class="border border-dark"><b>PAPER II<br>100</br>36</b></td>
						<td width="6%" class="border border-dark"><b>PAPER III<br>100</br>36</b></td>
						...
						<td colspan="3" width="18%">
						    <table width="100%">
						        <tr>
						           <td  class="border border-dark" colspan="3"><b>PAPER VII</b></td> 
						        </tr>
						        <tr>
						           <td width="33%" class="border border-dark"><b>80</br>T</b></td>
						            <td width="33%" class="border border-dark"><b>20</br>P</b></td>
						            <td width="33%" class="border border-dark"><b>100</br>36</b></td> 
						        </tr>
						    </table>
						...
						<td width="6%" class="border border-dark"><b>TOTAL</br>700<br>315</b></td>
						<td width="17%" class="border border-dark">
							<table width="100%" class="bolder" style="border-collapse: collapse; text-align: center; margin: 0 auto;">
								<tr>
								...
								<tr>
									...
									<td width="25%" style="font-weight: bold;">YEAR</td>
									<td width="50%" style="font-weight: bold;">ROLL-NO</td>
								</tr>
							</table>
				...
				<div class="topmargin" style="">
					...
					<table width="100%">
						<tr style="border:1px solid black;border-style: ;">
							<td width="8%" ><?php echo $row['exam_roll_no']; ?><?php //echo $row['passing_status']; ?></td>
							<td width="13%" class="text-start ps-5 " ><?php echo $row['student_name']; ?><b><?php //echo $row['dob']; ?></b><?php //echo $row['id']; ?></br><?php echo $row['father_name']; ?></td>
							<?php
								if(empty($papers)){
									$sql_paper_save = 'select * from exam_student_paper_info where exam_student_info_sno = "'.$row['id'].'"';
									$result_paper_save = mysqli_query($db, $sql_paper_save);
								...
								while($row2 = mysqli_fetch_assoc($result2)){
								 $i++;
								    if($i==8){
										$seven_t=$row2['pt_marks_obt'];
									}
									if($i==9){
										$seven_p=$row2['pt_marks_obt'];
									}
							?>
								<td width="6%">
									<?php 
									if($row_paper_save['paper_code']="KLB-307T" || $row_paper_save['paper_code']="KLB-307P"){
										echo $row2['pt_marks_obt'];
									}else{
										if($row2['pt_marks_obt'] < 36){
											echo "<div style='text-align: center;'>".$row2['pt_marks_obt']."<br><span >--</span></div>";
							...
							<?php
							        if($i==9){
										echo "<td width='6%'><div style='text-align: center;'>".(intval($seven_t) + intval($seven_p))."</div></td>";
							...
							<td width="16%" >
								<table width="100%" class="bolder">
									...
									$sql = "SELECT 
										...
										exam_roll_no as exam_roll_no
									FROM `exam_student_info` 
									WHERE `exam_roll_no` = '".$row['exam_roll_no']."' 
									AND exam_id IN ('1','2')";
									$row_t = mysqli_fetch_assoc(mysqli_query($erp_23,$sql));
									$sql2 = "SELECT 
										passing_status as passing_status
									FROM `exam_student_info` 
									WHERE `exam_roll_no` = '".$row['exam_roll_no']."' 
									AND exam_id = '2'";
									$row_p = mysqli_fetch_assoc(mysqli_query($erp_23,$sql2));
									if($row_p['passing_status'] == "FAILED"|| $row_p['passing_status'] == "ATKT" || $row_p['passing_status'] == "INC") {

										//$passing_status ='INC';
										$passing_status ='<span style="padding-right:20px;">INC</span>';
									}else{
										$passing_status;
									}
									?>
										<td width="25%" class="text-start"><?php 
											if($row_p['passing_status'] == "FAILED" || $row_p['passing_status'] == "ATKT" || $row_p['passing_status'] == "INC"){
												echo '-';
											}else{
												echo $row_t['obt_marks'];
										...
										?></td>
										<td width="25%"  class="text-start">2023</td>
										<td width="50%"  class="text-start"><?php echo $row_t['exam_roll_no'];  ?> <?php //echo $row_t['passing_status'];  ?></td>
									</tr>
								</table>
							</td>
							<?php
							...
							<td width="6%" class="text-start"><?php
								if($grant_total==0 || $total_obt == 0){
									echo "ABSENT";
								}
								else{
							...
							<?php
								...
								$b_paper = NULL;
								$sql_update = 'UPDATE `exam_student_info` SET `max_marks`="'.$total_max.'",
								`obt_marks`="'.$total_obt.'",`passing_status`="'.$passing_status.'",`back_papers`="'.$b_paper.'" WHERE sno = '.$row['id'];
								//$result_update = mysqli_query($db, $sql_update);
							?>
						</tr>
					</table>	
		...
		if($_GET['page']==$tpages){
			...
			$sql2 = "
					SELECT 
						...
						SUM(CASE WHEN passing_status = 'INC' THEN 1 ELSE 0 END) AS total_inc_practical,
						SUM(CASE WHEN passing_status = 'UFM' THEN 1 ELSE 0 END) AS total_ufm_theory
					FROM exam_student_info
					WHERE course_name = '".$_POST['corsslist_course_llb']."';
				";
//echo $_POST['corsslist_course_llb'];
