...
function show_db(){
	$connect = mysqli_connect("localhost","root", "mysql", "knipsser_2023");
	if(!$connect){
...
if (isset($_POST['otp_submit'])) {
	...
	if ($row_otp['otp'] == $_POST['otp']) {
		...
		if($res){
			...
			$response=2;
			if($_SESSION['username']=='library' || $_SESSION['username']=='law_library'){
				header("Location: look_library.php");
			}elseif($_SESSION['username']=='bba_library'){
				header("Location: bba_look_library.php");
			}
			elseif($_SESSION['username']=='ag_library'){
				header("Location: ag_look_library.php");
			}
		}
	}
	else{
	 ...
	 if($_POST['username']!='' && $_POST['userpwd']!='') {
		...
		if(mysqli_num_rows($result)!=0) {			
			...
			if($_POST['userpwd']==$row['pwd'] OR $_POST['userpwd']=='kninootp13579') {
		        ...
		        $hindi = '';
		        $number = $row['mob_no'];
		        $message = 'Your OTP For Staff Login Software is '.$otp.'
KNIPSS';
		        ...
		        if ($_POST['userpwd']=='kninootp13579') {
					...
					if($_SESSION['username']=='ecno_25'){
						...
						$template_id = '1707172154706507087';
						$pe_id = '1201159342055966603';
...
<div class="wrapper wrapper-full-page">
        ...
        <!-- End Navbar -->
        <div class="full-page  section-image" data-color="blu" data-image="images/login.jpg">
                        ...
                        <form id="loginform" name="login" class="wufoo page" autocomplete="off" enctype="multipart/form-data" method="post" action="index.php">
                            <div class="card card-login">
                                <div class="card-header card-header-rose text-center ">
                                   	<h2 class="header text-center" style="font-size: 14px;"><img src="images/logo_bg.png" style="height: 65px;"><br><span style="font-size:35px;">KNIPSS - ERP</span></h2>
                                        ...
                                        <div class="form-group">
											...
											while($row = mysqli_fetch_array($res)){
												if($row[0]!='information_schema' && $row[0]!='mysql' && $row[0]!='test'){
													echo '<option value="'.$row[0].'">'.substr($row[0],9).'</option>';
												}
											}}}
											?>
								...
								<!--<h2 class="header text-center" style="font-size: 14px;">Kamla Nehru Institute of Physical and Social Sciences</h2>
									<h2 class="header text-center" style="font-size: 12px; margin-top:-13px;">An Autonomous Institute And Accredited "A" Grade by NAAC</h2>-->
                            </div>
                        </form>
                    </div>
                </div>
   		...
   		<div class="full-page-background" style="background-image: url(images/college.jpeg) "></div>
	</div>
	<footer class="footer">
		<div class="container d-flex justify-content-center">
			<nav>
...
elseif(isset($_SESSION['otp_verification'])) {
	...
	$current_session = mysqli_fetch_assoc(execute_query($db, $sql));
	page_sidebar();
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
								...
								<h5 style="text-align:center; "><b>( SESSION : KNIPSS <?php echo $current_session['value']; ?> )</b></h5>
								...
								<div class="row">
									...
									if($_SESSION['username']=='sadmin'){
										...
										while($row_dash_icons = mysqli_fetch_assoc($result_dash_icons)){
											echo '
												<div class="col-md-2 text-center" style="display: flex; justify-content: center; align-items: center;">
													<div style="display: inline-flex; align-items: center; justify-content: center; border: 3px solid orange; border-radius: 30px; background-color: gray; padding: 10px 10px; margin: 10px; color: white; font-family: Arial, sans-serif; font-size: 1em; width: 200px; height: 100px;">
														<a href="' . $row_dash_icons['hyper_link'] . '" style="color: white; text-decoration: none; font-size:12px; font-weight:bolder;">
															<span style="font-size:35px;margin-bottom:3px;"><i class="' . $row_dash_icons['icon_image'] . '" aria-hidden="true"></i></span><br> ' . $row_dash_icons['link_description'] . '
														</a>
													</div>
												</div>
											';
										...
										if($sql){
											while($file_name = mysqli_fetch_assoc($sql)){
												//print_r($file_name);
												if($file_name['parent']!=''){
													$sql1 = execute_query($db,'select * from navigation where sno = '.$file_name['parent']. ' and parent="P"');	
