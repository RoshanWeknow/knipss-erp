...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE lib_add_building set 
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO lib_add_building(library_name,building_name, created_by, creation_time) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from lib_add_building where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from lib_add_building where sno = '.$_GET['edit'];
						...
						<tr class="bg-primary text-white">
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td><a href="lib_add_building.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="lib_add_building.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
