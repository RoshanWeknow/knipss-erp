...
<?php
	if(isset($_POST['submit'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			...
			if(move_uploaded_file($_FILES['attach_file']['tmp_name'], $targetFile)){
				$sql = 'insert into events (title, name, department, venue, start_date, end_date, files, description, remarks, created_by, created_date, edited_by, edited_date) 
						values
					   ("'.$_POST['title'].'",
						"'.$_POST['name'].'",
						"'.$_POST['department'].'",
	...
	if(isset($_GET['del'])){
		$sql = 'delete from events where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from events where sno = '.$_GET['edit'];
		...
		<div class="card card-body">    
        	<div class="row d-flex my-auto">  
				<form action="" class="wufoo leftLabel page1" name="" enctype="multipart/form-data" method="POST" onSubmit="" >
					<div class="bg-primary text-white p-2"><h3>Add Event</h3></div>
						<div class="col-md-12" >
							...
							<table width="100%" class="table table-striped  table-hover rounded">
								...
								<tr >
									<th>Venue</th>
									<th><input type="text" name="venue" id="venue" value="<?php echo isset($_GET['edit'])? $res['venue']: '' ?>" class="form-control"></th>
									<th>Start Date</th>
									<th><script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $res['start_date']: date('Y-m-d');
												echo "document.writeln(DateInput('start_date', 'start_date', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script></th>
									...
									<th><script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $res['end_date']: date('Y-m-d');
												echo "document.writeln(DateInput('end_date', 'end_date', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script></th>
								</tr>
								<tr>
						...
						<tr class="bg-primary text-white" >
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td>'.$row['remarks'].'</td>
									<td><a href="events.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="events.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
