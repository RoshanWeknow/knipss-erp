...
if(isset($_POST['submit'])){
	...
	$sql = 'delete from fees_detail3 where class_id="'.$class_id.'"';
	...
	if($_POST['class_type']!='SELF'){
		...
		$sql_fees='insert into fees_detail3(class_id, head_id,fee_amount) value
	...
	if($msg==''){
		...
		$sql = "delete from subject_fees3 where class_id=".$class_id;
		execute_query(connect(), $sql);
		...
		$sql='insert into subject_fees3(class_id,subject_id,fees,type,code) values';
...
if(isset($_GET['del'])){
	$sql = 'delete from subject_fees3 where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
	
	$sql = 'delete from fees_detail3 where class_id='.$_GET['del'];
	execute_query(connect(), $sql);
        	...
        	<div class="row d-flex my-auto">    	
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" class="wufoo leftLabel page1" name="addnewclass" enctype="multipart/form-data"
                 method="post" onSubmit="" >
    				<h2><img style="width:40px;" src="images/add.png" />Edit <span class="orange">Class (SI SC/ST)</span></h2>
					<?php
                    switch($response){
                        case 1: {
                    ?>
					...
					<?php echo $msg;?>
								...
								<table width="100%" class="table table-striped table-hover rounded">
									...
									while($row = mysqli_fetch_array($result)) {
									  ...
									  if($z%2!=0){
										...
										<td>'.$row['category'].'</td>
										<td>'.$row['type'].'</td>
										<td><a href="edit_class3.php?id='.$row['sno'].'">Edit</a></td>
										<td><a href="edit_class3.php?del='.$row['sno'].'">Delete</a></td>
										</tr>';
									}  
									?>
								</table>
							...
							<?php
								...
								if(mysqli_num_rows($class_result)!=1){
									...
									$sql='select * from fees_detail3 where class_id="'.$class_row['sno'].'" and head_id="computer"';
									$computer=mysqli_fetch_array(execute_query(connect(), $sql));
									$sql='select * from fees_detail3 where class_id="'.$class_row['sno'].'" and head_id="self"';
									$self=mysqli_fetch_array(execute_query(connect(), $sql));
									$sql='select * from fees_detail3 where class_id="'.$class_row['sno'].'" and head_id="tour"';
   					...
   					<div class="card card-body" style="display: none;" id="heads">
						...
						<table width="100%" class="table table-striped table-hover rounded">
							...
							while($row = mysqli_fetch_array($res)) {
								$sql = "select * from `fees_detail3` where class_id = '".$class_row['sno']."' and head_id = '".$row['sno']."'";
					...
					<div class="card card-body">
						...
						<fieldset style="width:950px;" class="table table-striped table-hover rounded">	
								...
								while($row1 = mysqli_fetch_array($res1)) {
									$sql = 'select * from subject_fees3 where class_id='.$_GET['id'].' and subject_id='.$row1['sno'];
