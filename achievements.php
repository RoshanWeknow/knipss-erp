...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        ...
        if(move_uploaded_file($_FILES['attach_file']['tmp_name'], $targetFile)){
            $sql = 'insert into achievements (title, name, department, date, files, description, remarks, created_by, created_date, edited_by, edited_date) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from achievements where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from achievements where sno = '.$_GET['edit'];
...
<div id="container">	
		<div class="card card-body">    
        	<div class="row d-flex my-auto">  
				<form action="achievements.php" class="wufoo leftLabel page1" name="" enctype="multipart/form-data" method="POST" onSubmit="" >
					<div class="bg-primary text-white p-2"><h3>Add Achievements</h3></div>
						<div class="col-md-12" >
							<table width="100%" class="table table-striped  table-hover rounded">
								...
								<tr >
									...
									<th>Attach File</th>
									<th><input type ="file" id="attach_file" name="attach_file" value="<?php echo isset($_GET['edit'])? $res['file']: '' ?>" class="form-control"></th>
									<th>Description</th>
									<th><textarea type="text" id="description" name="description" value="<?php echo isset($_GET['edit'])? $res['description']: '' ?>" class="form-control"></textarea></th>
								</tr>
								<tr>
									<th>Remarks</th>
									<th><textarea type="text" id="remarks" name="remarks" value="<?php echo isset($_GET['edit'])? $res['remarks']: '' ?>" class="form-control"> </textarea></th>
								</tr>
							</table>
						...
						<tr class="bg-primary text-white">
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td>'.$row['remarks'].'</td>
									<td><a href="achievements.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="achievements.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
