...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE bba_lib_add_building set 
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO bba_lib_add_building(library_name,building_name, created_by, creation_time) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from bba_lib_add_building where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from bba_lib_add_building where sno = '.$_GET['edit'];
				...
				<div class="col-md-12" >
					<table width="80%" class="table table-striped  table-hover rounded">
						<tr class="bg-primary text-white">
							...
							<?php
								$sql = 'select * from bba_lib_add_building';
								$result = mysqli_query($db, $sql);
								$i=1;
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>
									...
									<td><a href="bba_lib_add_building.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="bba_lib_add_building.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
