...
if(isset($_GET['del'])){
	if($_SESSION['type']=='sadmin'){
		$sql = 'delete from mid_sem_marks_allotment where sno="'.$_GET['del'].'"';
		execute_query($db, $sql);
		
		$sql = 'update exam_student_paper_info set mid_sem_allotment=0, mid_sem_marks_max=NULL, mid_sem_marks_obt=NULL, mid_teacher=NULL, mid_verifier=NULL where mid_sem_allotment ="'.$_GET['del'].'"';
		execute_query($db, $sql);
	...
	<body>
		<div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
					<h3>Mid Sem Allotment Report</h3>
					...
					<table class="table table-striped table-hover" id="general_stat_table">
						...
						<?php
							$query = 'SELECT * FROM mid_sem_marks_allotment';
							if($_POST['semester']=='odd'){
								
								$query = 'SELECT t1.sno, class_id, class_description, `year`, t1.max_marks as max_marks, examiner, v_auth, paper, stu_count, allot_stu, title_of_paper FROM mid_sem_marks_allotment t1
							...
							while($row=mysqli_fetch_assoc($result)){
								...
								$sql_chk_marks_feed = 'select * from exam_student_paper_info WHERE mid_sem_allotment = "'.$row['sno'].'" AND paper_code = "'.$row['paper'].'"';
								$res_chk_marks_feed = mysqli_fetch_assoc(mysqli_query($db, $sql_chk_marks_feed));
								//echo $sql_chk_marks_feed;
								if($res_chk_marks_feed['mid_sem_marks_obt'] != ''){
									$feeded = 1;
								}
								else{
									$feeded = 0;
						...
						<tr>
							...
							<td><?php echo $row['title_of_paper']?> (<?php echo $row['paper'];?>)</td>
							...
							<td><a href="exam_mid_sem_alloted_stu_report.php?view=<?php echo $row['sno'];?>&ppr=<?php echo $row['paper'];?>" target="_blank">View</a></td>
							<?php 
							if($feeded == 0){
							?>	
							<td><a href="exam_mid_sem_marks_entry.php?id=<?php echo $row['sno'];?>&ppr=<?php echo $row['paper'];?>" target="_blank"	style="color:green;">Feed Theory Marks</a></td>
							<?php
								 }
							 elseif($feeded == 1){
							?>	
							<td><a href="exam_mid_sem_marks_report.php?id=<?php echo $row['sno'];?>&ppr=<?php echo $row['paper'];?>" target="_blank"	style="color:red;">Marks Report</a></td>
							<?php
							}
							if($_SESSION['type']=='sadmin'){
								echo '<td><a href="exam_mid_sem_marks_allotment_report.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure?\');" class="text text-danger">Delete</a></td>';
							}
							?>
						</tr>
						<?php
