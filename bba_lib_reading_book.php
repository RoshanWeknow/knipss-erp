...
<?php
	if(isset($_POST['submit2'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			...
			$query = 'select * from bba_lib_reading_book where accession_no = "'.$_POST['accession_no'].'" AND status =1';
			$res = execute_query($db,$query);
			if(mysqli_num_rows($res)==0){
				$sql = 'insert into bba_lib_reading_book (lib_card_no, stu_name, mobile, accession_no, subject, author_name, booksissuedate, created_by, creation_time)
	...
	if(isset($_GET['del'])){
		$sql = 'delete from bba_lib_reading_book where sno="'.$_GET['del'].'"';
		execute_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from bba_lib_reading_book where sno = '.$_GET['edit'];
