...
switch($_SESSION['re_class_type']){
			case 'ballb': {
				$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_SESSION['re_from_date'].'" AND `approval_date` <= "'.$_SESSION['re_to_date'].'" and class_detail.sort_no="BA_LLB" order by fee_invoice.sno';
				
				break;
			}
			case 'AIDED' : {
				$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_SESSION['re_from_date'].'" AND `approval_date` <= "'.$_SESSION['re_to_date'].'" and class_detail.type!="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
				break;
			}
			case 'SELF' : {
				$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where `approval_date` >= "'.$_SESSION['re_from_date'].'" AND `approval_date` <= "'.$_SESSION['re_to_date'].'" AND class_detail.type="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
