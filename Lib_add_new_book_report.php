                          ...
                          <div class="row">
                            <div class=" col-md-4 ">
                                ...
                                <select name="course" id="course" value="" class="form-control" tabindex="<?php echo $tabindex++;?>" required>
									...
									<?php 
										//$sql  = 'select * from lib_old_book_info group BY Subject_Heading';
										// $sql = 'SELECT * FROM lib_old_book_info GROUP BY col2 ORDER BY col2 DESC';
										// $dept_list = execute_query($db, $sql);
										// if($dept_list){
											// while($list = mysqli_fetch_assoc($dept_list)){
												// echo '<option  value = "'.$list['col2'].'">'.$list['col2'].'</option>';
											// }
										// }
									?>
								</select>
...
<div id="container">
	<?php if(isset($_POST['course'])){
			echo "<h5 style='color:green;' align='right'	>TOTAL ".$_POST['course'] ." BOOKS : ".$num_data."</h5>";
		} 
	?>
	<div class="card card-body">
		...
		<?php	
			if(isset($_POST['submit']) && $_POST['subject'] != ''){
				$sql = 'SELECT * FROM lib_add_new_book WHERE col2 = "'.$_POST['course'].'"';
			}
			else{
				$sql = 'select * from lib_add_new_book';
			}	
