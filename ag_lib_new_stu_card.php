...
if(isset($_POST['submit2'])){
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		...
		if(mysqli_errno($conn)){
			...
			if($res["error"]!=0){
				$sql = "update ag_lib_id_card set profile = '". $res["file_name"]."' where sno = ".$id;
				//echo $sql."<br>";
				mysqli_query($conn,$sql);
				if(mysqli_errno($conn)){
		...
		if (isset($_POST["name"])) {
			...
			if($res["error"]!=0){
				$sql = "update ag_lib_id_card set qr_code = '". $filename."' where sno = ".$id;
				//echo $sql;
				mysqli_query($conn,$sql);
				if(mysqli_errno($conn)){
