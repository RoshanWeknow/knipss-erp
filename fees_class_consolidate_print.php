...
$end=$_SESSION['end1'];
$_POST['dfc_date']=$_SESSION['fees_date1'];
?>
<style>
body{
	width: 400mm;
...
<input type="hidden" name="total_student" value="'.$tot_stu_count.'"></td>';
	...
	while($a = mysqli_fetch_array($re)){
		if($a['sno']==1){
			echo '<td style="width:80px;">'.$grand_tot[$a['sno']].'<input type="hidden" name="fees_total_'.$a['sno'].'" value="'.$grand_tot[$a['sno']].'"></td>';
		}
		else{
			echo '<td style="width:70px;">'.$grand_tot[$a['sno']].'<input type="hidden" name="fees_total_'.$a['sno'].'" value="'.$grand_tot[$a['sno']].'"></td>';
		}
		if($a['sno']==3){
			echo '<td  style="width:80px;">'.$grand_tot[50].'</td>';
		}
		if($a['sno']==21){
			echo '<td  style="width:80px;">'.$grand_tot[22].'</td>';
		}
	}
	echo '<td  style="width:80px;">'.$grand_tot[52].'</td>';
	echo '<td  style="width:80px;">'.$grand_tot[30].'</td>';
	//echo '<td  style="width:80px;">'.$grand_tot[31].'</td>';
