...
<?php	
	if(isset($_POST['submit2'])){
		...
		$sql = 'UPDATE `ag_lib_issue_book` SET `status`=0,
	...
	<div class="card card-body">    
		<div class="row d-flex my-auto">  	
			<div class="d-flex  justify-content-between">
				...
				<div><a href="ag_lib_book_return_record.php" style="margin-right:2rem;" class="btn btn-info" >Full Report</h3></a></div>
			</div>
			<div class="col-md-12" >
				<table width="100%" class="table table-striped  table-hover rounded">
					<tr class="bg-primary text-white" align="center">
						...
						<?php
							// if(isset($_POST['accession_no']) && ($_POST['accession_no'] != '')){
								// echo $query = 'SELECT * FROM ag_lib_issue_book WHERE accession_no = "'.$_POST['accession_no'].'" AND status = 1';
							// }
							...
							// }
							// else{
								// echo $query = 'SELECT * FROM ag_lib_issue_book WHERE status = 1';
							// }
							
							$query = 'SELECT * FROM ag_lib_issue_book WHERE status = 1';
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								echo '<tr align="center">
									...
									<td><a href="ag_lib_book_return.php?reissue='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');"><h3 class="btn btn-sm btn-success">ReIssue</h3></a></td>';?>
									
									<?php
									if(isset($_POST['fine_submit'])){
										$sql='UPDATE `ag_lib_issue_book` SET 
									...
									?>
									<?php echo'
									<form action="ag_lib_book_return.php" method="POST">
										<td>
