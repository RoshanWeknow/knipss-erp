...
<?php
	...
	if(isset($_POST['submit'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			$sql = 'update institute_directory set 
					title="'.$_POST['title'].'" ,
					name="'.$_POST['name'].'" ,
			...
			$sql = 'insert into institute_directory (title, name, contact_details, description, remarks, created_by, created_date, edited_by, edited_date) 
					values
				   ("'.$_POST['title'].'",
					"'.$_POST['name'].'",
	...
	if(isset($_GET['del'])){
		$sql = 'delete from institute_directory where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from institute_directory where sno = '.$_GET['edit'];
		...
		<div class="card card-body">    
        	<div class="row d-flex my-auto">  
				<form action="" class="wufoo leftLabel page1" name="" enctype="multipart/form-data" method="POST" onSubmit="" >
					<div class="bg-primary text-white p-2"><h3>Add/Update Directory</h3></div>
						<div class="col-md-12" >
							<table width="100%" class="table table-striped  table-hover rounded">
								<tr >
									...
									<th><input type="text" name="contact_details" id="contact_details" value="<?php echo isset($_GET['edit'])? $res['contact_details']: '' ?>"  class="form-control"></th>
								</tr>
								<tr>	
									<th>Description</th>
				...
				<div class="col-md-12" >
					<table width="100%" class="table table-striped  table-hover rounded">
						<tr class="bg-primary text-white">
							...
							<?php
								$sql = 'select * from institute_directory';
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td>'.$row['remarks'].'</td>
									<td><a href="institute_directory.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="institute_directory.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
