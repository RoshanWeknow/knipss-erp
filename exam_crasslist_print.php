		...
		<div class="wrap">
					...
					<tr>
						<td>
							<div style="width:100%" class="border ">
							    <!--class="noprint"--->
								<div  class=""><h4 class="" style="text-align: center; margin:0px; " ><span ><b>Kamla Nehru Institute Of Physical &amp; Social Sciences,Sultanpur, Uttar Pradesh</b></span> <br><span style="font-size:1.3rem;">An Autonomous Institute And Accredited "A" Grade by NAAC</span></h4></div>
								<div class="d-flex">
									<?php
										...
										//print_r($row);
									?>
									<div class="class_name"><b><?php echo $row_class['class_description']?>  Main Examination <?php
										$db_year = substr($_SESSION['db_name'], -4);
										$next_year = $db_year + 1;
										echo $db_year . "-" . $next_year;
										?>
									</b></div>
									<div style="margin-left:20px;font-weight:bolder;">Regular/Supplementary</div>
				...
				<tbody>
					<tr>
						...
						for ($pgid = $start; $pgid < $end; $pgid++) {
							...
							$i = $pgid+1;
							if($exam_type!=$row['exam_type']){
    							if($exam_type!='' && strtolower($row['exam_type'])=='regular'){
    							    echo '<div class="text-center" style="font-size:13px;"><b>SUMMARY (Supplementary/Ex-Student/Back-Student)</b></div>';
    							    echo '<table width="100%" class="table table-bordered" style="border:1px solid black;">
                        				<tr class="text-center">
                        					<th width="16%" class="report_border"> Course Name</th>
                        					...
                        					<th width="9%" class="report_border">ATKT</th>
                        					<th width="9%" class="report_border">FAIL</th>
                        					<th width="9%" class="report_border">UFM</th>
                        					<th width="9%" class="report_border">INC</th>
                        				</tr>
                        				<tr>
                                			<th  class="report_border">'.$row_class['class_description'].'</th>
                                			<td class="report_border"></td>
                                			<td class="report_border"></td>
                                			<td  class="report_border"></td>
                                			<td class="report_border"></td>
                                			<td class="report_border"></td>
                        			...
                        			<div class="" style="display:flex;justify-content:space-between;font-size:13px;">
												<div><b>DATE OF RESULT DECLRATION :   '.date("d-m-Y").'</b></div>
												<div><b>SIGNATURE OF CONTROLLER OF EXAMS :</b></div>
												<div><b>CO-ORDINATOR :</b></div>
											</div>';
                        			
                        			echo '<div style="page-break-after: always;margin-top:2rem;"></div>';
			...
			$sql2 = "
					SELECT 
						...
						SUM(CASE WHEN passing_status = 'ATKT' THEN 1 ELSE 0 END) AS total_pass_atkt,
					...
					FROM exam_student_info
					WHERE course_name = '".$_POST['corsslist_course']."' AND verify_status='1';
				";
//echo $_POST['corsslist_course'];
		...
		?>
			...
			<table width="100%" class="table table-bordered" style="border:1px solid black;">
				...
				<tr>
					...
					<td class="report_border"><?php  echo $result_row['total_pass_atkt']; ?></td>
		...
		?>
											<div class="" style="display:flex;justify-content:space-between;font-size:13px;">
												<div><b>DATE OF RESULT DECLRATION :  <?php 
    ...
    echo 'N/A'; // or an empty string or some default message
}
