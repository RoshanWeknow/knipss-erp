...
$run=execute_query(connect(), $sql);
	...
	<tr class=""table-primary>
		...
		$sql .= ' GROUP BY `class`';
		$_SESSION['form_fee_month_wise'] = $sql;
		//echo $sql;
              $i=1;
              $amount = 0;
              $count = 0;
	...
	<div id="container">	
		<div class="card card-body">    
        	<div class="row d-flex my-auto">
				<div class="print-only btn btn-success text-white" value="">
				<h4><?php if(isset($_POST['date_from'])){echo date("d-m-Y" , strtotime($_POST['date_from']));}else{echo date("d-m-Y");} ?> to <?php if(isset($_POST['date_to'])){echo date("d-m-Y" , strtotime($_POST['date_to']));}else{echo date("d-m-Y");} ?></h4></div>
				<form action="form_fee_report_month_wise.php" class="wufoo leftLabel page1" name="feesdeposit" enctype="multipart/form-data" method="post" onSubmit="" autocomplete="off" >
				<h2><!--Manual Miscellaneous <span class="orange">Fee</span>--></h2>
					<div class="no-print">
						...
						<div class="col-md-12">
							...
							<a href="form_fee_report_month_wise_print.php" class="submit btn-danger btn " target="_blank" style="float:right;margin-right: 25px;padding-top: 10px;width: 80px;">Print</a>
							<?php echo $msg;?>
						</div>
					</div>
				</form> 
