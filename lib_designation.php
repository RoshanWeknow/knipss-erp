...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE lib_designation set 
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO lib_designation(institute_name, library_name, category, d_name, created_by, created_date) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from lib_designation where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from lib_designation where sno = '.$_GET['edit'];
	...
	<div class="card">
		<div class="card-body ">
			<div class="row d-flex my-auto">
				...
				<div class="bg-primary text-white p-2"><h3>Designation</h3></div>
					<table width="100%" class="table table-striped table-hover rounded">
						<tr class="">
							...
							<th><select class="form-control" name="library_name" id="library_name" class="form-control">
									<option value="">----Select----</option>
									<option value="KNIPSS(CENTRAL LIBRARY)"<?php echo (isset($_GET['edit']) && $res['library_name']=='KNIPSS(CENTRAL LIBRARY)') ?' selected="selected"':''  ?>>KNIPSS(CENTRAL LIBRARY)</option>
								</select></th>
						...
						<tr>
							...
							<th><input type="text" name="d_name" id="d_name" value="<?php echo isset($_GET['edit'])? $res['d_name']: '' ?>" class="form-control" required="required"></th>
						</tr>
					</table>
						...
						<tr class="bg-primary text-white">
							...
							<?php
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td>'.$row['category'].'</td>
									<td>'.$row['d_name'].'</td>
									<td><a href="lib_designation.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="lib_designation.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
