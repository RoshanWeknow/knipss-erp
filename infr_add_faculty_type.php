...
<?php
	if(isset($_POST['campus_name'])){
		if(isset($_POST['edit']) && $_POST['edit'] != ''){
			$sql = 'update infr_add_faculty_type set 
			...
			$sql = 'insert into infr_add_faculty_type (campus_name, faculty_type, created_by, creation_time ) 
					values("'.$_POST['campus_name'].'","'.$_POST['faculty_type'].'","'.$_SESSION['username'].'","'.date('Y-m-d H:m:s').'")';
			//echo $sql;
			execute_query($db,$sql);
			if(mysqli_errno($db)){
	...
	if(isset($_GET['del'])){
		$sql = 'delete from infr_add_faculty_type where sno="'.$_GET['del'].'"';
		execute_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
		$sql = 'select * from infr_add_faculty_type where sno = '.$_GET['edit'];
                    ...
                    <div class="col-md-12">
						...
						<table width="100%" class="table table-striped table-hover rounded">
							<tr >
								...
								<th width="15%"><select name="campus_name" id="campus_name" value="<?php echo isset($_GET['edit'])? $res['campus_name']: ''?>" class="form-control" required>
			...
			<table width="100%" class="table table-striped table-hover rounded">
				...
				<?php
					...
					while($row = mysqli_fetch_assoc($result)){
						...
						<td><a href="infr_add_faculty_type.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
						<td><a href="infr_add_faculty_type.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
							</tr>'	;
					}
								?>
			</table>
