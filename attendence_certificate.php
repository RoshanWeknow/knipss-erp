...
if(isset($_POST['enrollment_no']) && $_POST['enrollment_no'] != ''){
	...
	if(isset($_POST['edit']) && $_POST['edit']!= ''){
		$sql = 'update ct_attendence_certificate set 
		...
		$sql = 'insert into ct_attendence_certificate(enrollment_no,name,institution_name,course_name,course_start_date,course_end_date,issue_date,description,created_by,creation_time) values(
		"'.$_POST['enrollment_no'].'","'.
		$_POST['name'].'","'.
		$_POST['institution_name'].'","'.
		$_POST['course_name'].'","'.
		$_POST['course_start_date'].'","'.
		$_POST['course_end_date'].'","'.
		$_POST['issue_date'].'","'.
		$_POST['description'].'","'.
		$_SESSION['username'].'","'.
		date('Y-m-d H:m:s').'")';
...
if (isset($_GET['edit'])) {
	$sql = 'select * from ct_attendence_certificate where sno=' . $_GET['edit'];
	$data = mysqli_fetch_assoc(execute_query($db,$sql));
}

if(isset($_GET['del']) and $_GET['del']!='') {
		$sql = 'delete from ct_attendence_certificate where sno=' . $_GET['del'];
							...
							<div class="row">
								<div class="col-md-3 ">
									<div class="form-group">
										...
										<script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $data['course_start_date']: date('Y-m-d');
												echo "document.writeln(DateInput('course_start_date', 'user_form', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script>
									</div>
								</div>
								<div class="col-md-3 ">
									<div class="form-group">
										...
										<script type="text/javascript" language="javascript">
											<?php
												$date = isset($_GET['edit'])? $data['course_end_date']: date('Y-m-d');
												echo "document.writeln(DateInput('course_end_date', 'user_form', true, 'YYYY-MM-DD','$date', 1));"
											?>
										</script>
									</div>
								</div>
		...
		<div class="card card-body">
			<table  class="table table-striped table-hover rounded">
				...
				<?php
					...
					$sql = 'select * from ct_attendence_certificate';
