...
<?php
	if(isset($_POST['submit'])){
		$sql = 'SELECT *,exam_student_paper_info.sno as stu_paper_id FROM `exam_student_paper_info` LEFT JOIN exam_student_info on exam_student_info_sno=exam_student_info.sno  WHERE mid_sem_allotment ="'.$_POST['alloted_id'].'" AND paper_code ="'.$_POST['ppr'].'" ORDER BY exam_roll_no';
		$res = mysqli_query($db, $sql);
		while($row = mysqli_fetch_assoc($res)){
			if($_POST['obt_marks_'.$row['stu_paper_id']]!=''){
				$update_sql = "UPDATE exam_student_paper_info SET 
				mid_sem_marks_max='" .$_POST['max_marks_'.$row['stu_paper_id']]."',
				mid_sem_marks_obt='" .$_POST['obt_marks_'.$row['stu_paper_id']]."'
            ...
            <div class="col-md-12">
				...
				<?php echo $msg; ?>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']?>" enctype="multipart/form-data" method="POST" onSubmit="" autocomplete="off">
							<td><input type="text" name="max_marks_<?php echo $row['stu_paper_id']?>" value="<?php echo $row['mid_sem_marks_max']?>" class="form-control" readonly></td>
							<td><input type="text" name="obt_marks_<?php echo $row['stu_paper_id']?>" value="<?php echo $row['mid_sem_marks_obt']?>" class="form-control"></td>
