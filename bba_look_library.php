	...
	<div class="col-md-12 bg-light">
		<div class="content bg-light " style="over-flow:hidden;">
			<div style="--bs-gutter-x: -73px !important;overflow-x:hidden !important;margin-start:30px;" class="row">
				<?php
				if($_SESSION['username']=='library' || $_SESSION['username']=='llb_library' || $_SESSION['username']=='sadmin' || $_SESSION['username']=='law_library'){
					//$sql = 'select * from navigation where parent="167" order by sort_no';
					$sql = 'SELECT * FROM navigation WHERE parent="167" ORDER BY ABS(sort_no)';
				...
				elseif($_SESSION['username']=='bba_library' ){
					//$sql = 'select * from navigation where parent="167" order by sort_no';
					$sql = 'SELECT * FROM lib_navigation WHERE parent IN ("30","31","39","40","45") ORDER BY ABS(sort_no)';
					$result_dash_icons = execute_query($db, $sql);
					echo mysqli_error($db);
					while($row_dash_icons = mysqli_fetch_assoc($result_dash_icons)){
						echo '<div class="zoom " "><a href="'.$row_dash_icons['hyper_link'].'"  target="_blank">
					...
					if($sql){
						while($file_name = mysqli_fetch_assoc($sql)){
							//print_r($file_name);
							if($file_name['parent']!=''){
								$sql1 = execute_query($db,'select * from navigation where sno = '.$file_name['parent']. ' and parent="167"');	
