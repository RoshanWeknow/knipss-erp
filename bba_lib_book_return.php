...
<?php	
	if(isset($_POST['submit2'])){
		...
		$sql = 'UPDATE `bba_lib_issue_book` SET `status`=0,
	...
	if (isset($_GET['reissue'])) {
		$sql1 = 'SELECT * FROM `bba_lib_issue_book` WHERE sno = "'.$_GET['reissue'].'"';
		$result = execute_query($db, $sql1);
		$row = mysqli_fetch_assoc($result);
		$issueCount = $row['issue_count'] + 1;
		
		$sql = 'UPDATE `bba_lib_issue_book` SET `issue_count`= "'.$issueCount.'",
	...
	<div class="card card-body">    
		<div class="row d-flex my-auto">  	
			<div class="d-flex  justify-content-between">
				...
				<div><a href="bba_lib_book_return_record.php" style="margin-right:2rem;" class="btn btn-info" >Full Report</h3></a></div>
			</div>
			<div class="col-md-12" >
				<table width="100%" class="table table-striped  table-hover rounded">
					<tr class="bg-primary text-white" align="center">
						...
						<?php
							// if(isset($_POST['accession_no']) && ($_POST['accession_no'] != '')){
								// echo $query = 'SELECT * FROM bba_lib_issue_book WHERE accession_no = "'.$_POST['accession_no'].'" AND status = 1';
							// }
							// elseif(isset($_POST['lib_card_no']) && ($_POST['lib_card_no'] != '')){
								// $query = 'SELECT * FROM bba_lib_issue_book WHERE lib_card_no = "'.$_POST['lib_card_no'].'" AND status = 1';
							// }
							// elseif(isset($_POST['first_date']) && isset($_POST['last_date'])){
								// $startDate = $_POST['first_date'];
								// $endDate = $_POST['last_date'];
								// $query = 'SELECT * FROM bba_lib_issue_book WHERE booksissuedate BETWEEN "'.$startDate.'" AND "'.$endDate.'" AND status = 1';
							// }
							// else{
								// echo $query = 'SELECT * FROM bba_lib_issue_book WHERE status = 1';
							// }
							
							$query = 'SELECT * FROM bba_lib_issue_book WHERE status = 1';
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								echo '<tr align="center">
									...
									<td><a href="bba_lib_book_return.php?reissue='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');"><h3 class="btn btn-sm btn-success">ReIssue</h3></a></td>';?>
									...
									?>
									<?php echo'
									<form action="bba_lib_book_return.php" method="POST">
										<td>
