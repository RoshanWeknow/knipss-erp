...
function class_report() {
	window.open("fee_receipt_generation_first_installment_print.php");
}
</script>
<body id="public">
	<div id="container">	
		<div class="card card-body">    
        	<div class="row d-flex my-auto">    	
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="wufoo leftLabel page1" name="feesdeposit" enctype="multipart/form-data" method="post" onSubmit="" >
					<h2>Fees <span class="orange">Fee Receipt Generation(First Installment)</span></h2>
					<div class="col-md-12">
						<div class="row">							
							<div class="col-md-4">							
		...
		<div class="card card-body">
			<table width="100%" class="table table-striped table-hover rounded">
				...
				$receipt_number=0;
				$n=1;
				if(isset($_POST['generate']) & $_POST['start_receipt_number'] != ''){
					$sql_fee_type = 'SELECT * FROM `fee_invoice` group by `type`';
					...
					while($row_fee_type = mysqli_fetch_array($result_fee_type)){
						...
						switch($_POST['class_type']){
							case 'ballb': {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" AND `fee_invoice`.`type`="'.$row_fee_type['type'].'" and class_detail.sort_no="BA_LLB" order by fee_invoice.sno';
								
								break;
							}
							case 'AIDED' : {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" AND `fee_invoice`.`type`="'.$row_fee_type['type'].'" and class_detail.type!="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
								break;
							}
							case 'SELF' : {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" AND `fee_invoice`.`type`="'.$row_fee_type['type'].'" and class_detail.type="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
						...
						while($row_data = mysqli_fetch_assoc($result_data)){
								$sql_update = 'UPDATE `fee_invoice` SET `receipt_number`="'.$receipt_number.'" WHERE `sno`="'.$row_data['sno'].'"';
					...
					while($row_fee_type = mysqli_fetch_array($result_fee_type)){
						$receipt_number = $_POST['start_receipt_number'];
						$sql = 'SELECT * FROM `fee_invoice` WHERE `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" AND `type`="'.$row_fee_type['type'].'"';
						...
						while($row=mysqli_fetch_array($res)){
							...
							if($_POST['class_type'] == 'self' and $row_self['type'] == 'SELF' and $row_self['sort_no'] != 'BA_LLB'){
								$sql_update = 'UPDATE `fee_invoice` SET `receipt_number`="'.$receipt_number.'" WHERE `sno`="'.$row['sno'].'"';
								$res_update = execute_query(connect(), $sql_update);
								$receipt_number++;
							}
						...
						switch($_POST['class_type']){
							case 'ballb': {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" and class_detail.sort_no="BA_LLB" order by fee_invoice.sno';
								
								break;
							}
							case 'AIDED' : {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" and class_detail.type!="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
								break;
							}
							case 'SELF' : {
								$sql = 'select fee_invoice.sno from fee_invoice left join class_detail on class_detail.sno = fee_invoice.class_id where  `approval_date` >= "'.$_POST['from_date'].'" AND `approval_date` <= "'.$_POST['to_date'].'" AND class_detail.type="SELF" and class_detail.sort_no!="BA_LLB" order by fee_invoice.sno';
						...
						$result_data = execute_query(connect(), $sql);
						while($row_data = mysqli_fetch_assoc($result_data)){
							$sql = 'select * from `fee_invoice` where `sno`="'.$row_data['sno'].'"';
