...
$db = $db;
		...
		if($student_info['class']==196){
		    $sql = 'SELECT * FROM `exam_student_paper_info` where exam_student_info_sno='.$student_exam_info['sno'];
		    $i=1;
		    //echo $sql;
		    $papers = array();
		    $result_papers_env = mysqli_query($db, $sql);
		    while($row_paper_env = mysqli_fetch_assoc($result_papers_env)){
		        $sql = 'select * from add_subject_details where paper_code="'.$row_paper_env['paper_code'].'"';
		        $paper_details = mysqli_fetch_assoc(mysqli_query($db, $sql));
...
// Logic to check and assign photo and signature paths
...
} elseif (fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '.jpg')) {
    $photo = fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '.jpg');
    $sign = fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '_sign.jpg');
} elseif (fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '.jpeg')) {
    $photo = fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '.jpeg');
    $sign = fileExists("PHOTO/" . ceil($_GET['id'] / 1000) . '/' . $stu_student_info['sno'] . '_sign.jpeg');
...
<body class="w-100 m-auto">
    ...
    <img src="images/college_log.png" id="overlays"
        style=" z-index:-2;opacity:0.15;position: absolute;top: 50%;left: 50%;-ms-transform: translate(-50%, -50%);transform: translate(-50%, -50%); width:30%;"
        alt="overlay image">
            ...
            <div class="table-responsive ">
                ...
                <table width="100%" class="table table-bordered border border-dark " align="center">
					...
					<?php
							...
							$i=1;
							//print_r($class);
							if($class['sno']=='196' || $class['sno']=='209' || $class['sno']=='213' || $class['sno']=='225' || $class['sno']=='217' || $class['sno']=='221'){
								//exam_student_info
								$sql = 'select * from exam_student_paper_info where exam_student_info_sno="'.$exam_student_info['sno'].'" and theory_practical!="Practical"';
								//echo $sql;
								$result_papers = mysqli_query($db, $sql);
								while($row_papers = mysqli_fetch_assoc($result_papers)){
									$sql = 'SELECT * FROM `add_subject` where sno="'.$row_papers['subject_id'].'"';
									$subject = mysqli_fetch_assoc(mysqli_query($db, $sql));
									
									// echo '<tr ><td class="bolding">'.$i++.'</td>
									// <td class="bolding">'.$row_papers['type'].'</td>
									// <td class="bolding">'.$subject['subject'].'</td>
									// <td class="bolding">'.$row_papers['paper_code'].'</td>
									// <td class="bolding">'.$row_papers['title_of_paper'].'</td>
									// <td class="bolding">'.$row_papers['credit'].'</td>
									// </tr>';
								}
							}
							else{
								...
								foreach($papers as $key=>$val){
									foreach($val as $k=>$v){
										if($k!=='subject_name'){
										    if($student_info['class']=='105' || $student_info['class']=='213' || $student_info['class']=='217' || $student_info['class']=='221' || $student_info['class']=='225' || $student_info['class']=='209'){
    											if($v['theory_practical']=='Practical'){
    												goto skipprac;
    											}
    										}
											// echo '<tr ><td class="bolding">'.$i++.'</td>
							...
							foreach($val as $k=>$v){
								if($k!=='subject_name'){
									...
									if(mysqli_num_rows($scheme_result)!=0){
										...
										$time=$scheme['time'];
											if(($scheme['paper_code']=="KZ010101T" ||$scheme['paper_code']=="KZ030301T") && $stu_student_info['gender']=="M"){
												$time= "11:30";
											}else{
												$time=$scheme['time'];
											}
    									...
    									<th>'.($scheme['shift']=='1'?'Morning':'Afternoon').' '.$time.'</th>';
    ...
    <br><br><br>
    <h6 style="text-align:right;margin-top:5rem;margin-right:1rem;margin-bottom:0;">Signature of Centre Superintendent
        With Seal</h6>
</body>
