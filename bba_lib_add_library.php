...
<?php	
    ...
    if(isset($_POST['edit']) && $_POST['edit'] != ''){
        $sql = 'UPDATE bba_lib_add_library set 
			 ...
			 //echo $sql;		 
				...
				$sql = 'INSERT INTO bba_lib_add_library(library_name, created_by, creation_time) 
	...
	if(isset($_GET['del'])){
		$sql = 'delete from bba_lib_add_library where sno="'.$_GET['del'].'"';
		mysqli_query($db, $sql);
		if(mysqli_error($db)){
	...
	if(isset($_GET['edit'])){
	$sql = 'select * from bba_lib_add_library where sno = '.$_GET['edit'];
				...
				<form action="<?php echo $_SERVER['PHP_SELF']?>" class="wufoo leftLabel page1" name="feesdeposit"  enctype="multipart/form-data" method="post" onSubmit="" autocomplete="off">
					<table width="100%" class="table table-striped table-hover rounded">
						<tr>	
							...
							<th width="20%"><input type="text" name="library_name" id="library_name" value="<?php echo isset($_GET['edit'])? $res['library_name']: '' ?>" class="form-control" required="required">
							</th>
							<th width="65%"></th>
						</tr>
					</table>
				...
				<div class="col-md-12" >
					<table width="80%" class="table table-striped  table-hover rounded">
						<tr class="bg-primary text-white">
							...
							<?php
								$sql = 'select * from bba_lib_add_library';
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									<td><a href="bba_lib_add_library.php?edit='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color: #3066ec;"> Edit</h3></a></td>
									<td><a href="bba_lib_add_library.php?del='.$row['sno'].'" onClick="return confirm(\'Are you sure? \');" <h3 style="color:red;"></h3>Delete</a></td>
										</tr>'	;
								}
							?>
					</table>
