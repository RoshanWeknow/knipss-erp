        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-secondary">
                    <h4 class="card-title text-center text-white">Agriculture Library Books</h4>
                </div>
                <div class="card-body">
                    <?php
                    // Query for total book count
                    $sql1 = "SELECT COUNT(*) AS total_books FROM lib_add_new_book WHERE library_location = 'Agriculture Library'";
                    $result1 = execute_query($db, $sql1);

                    if ($result1 && $row1 = mysqli_fetch_assoc($result1)) {
                        $num_data = $row1['total_books'];
                        echo "<h5 style='color:green;' align='right'>Total <u style='color:red; font-weight: bold;'>Agriculture Library</u> Books: {$num_data}</h5><br>";
                    } else {
                        echo "<h5 style='color:red;' align='right'>Error fetching book count.</h5><br>";
                    }

                    // Query for paginated books
                    $sql = "SELECT * FROM lib_add_new_book WHERE library_location = 'Agriculture Library' LIMIT $offset, $limit";
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        ...
                        echo "<p class='text-center'>No records found for Agriculture Library.</p>";
                    }
                    ?>
                </div>
            </div>
