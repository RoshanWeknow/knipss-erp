...
if(isset($_POST['generate_dcr'])){
	...
	if(mysqli_error(connect())){
		$msg .= '<li>Error 1.1 >> '.mysqli_error(connect()).' >> '.$sql;
	}
	else{
	...
	if($msg==''){
		for($i=1; $i<=$_POST['id']; $i++){
			if($_POST['amount_'.$i]!=''){
				...
				execute_query(connect(), $sql);
				if(mysqli_error(connect())){
					$msg .= '<li>Error 2.'.$i.' >> '.mysqli_error(connect()).' >> '.$sql;
				}
			}
		}
	}
...
<body id="public">
		...
		<div class="card card-body">    
        	<div class="row d-flex my-auto">   
                <form action="dcr_report.php" class="wufoo leftLabel page1" name="feesdeposit" enctype="multipart/form-data" method="post" onSubmit="" >                
					<h2>DCR Report
					<a href="dcr_report_previous.php" class="btn btn-danger" style="float:right;">[Previous DCRs]</a>
					</h2>               	<?php echo $msg;?>
					<div class="col-md-12">
						<div class="row">							
							<div class="col-md-4">							
	...
	if(isset($_POST['report_type'])){
		...
		$sql = 'select * from ('.$sql.') as t1 order by approval_date, sno';
		//echo $sql;
		//die();
		$_SESSION['sql']="$sql";
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				if($row['info']==3 && $row['class_id']!=$row['class']){
					...
					if(mysqli_num_rows($stud2)!=0){
						...
						//echo $stud_sql2.' >> '.$row['roll_no'].' >> '.$row['class_description'].' >> <br>';
					}
				}
				...
				if($row['info']=='form_fee'){
					...
					if($form_fee_row['fee_submission_type']=='Research Inovation Fee'){
						echo '<td>Research Inovation Fee'.$adm_type.'</td>';
					}
					else{
				...
				elseif($row['fee_type']=='breakage'){
					...
					echo '<td style="color:red">'.$row['fee_type'].'</td>';
				}
				...
				//echo '<h1>'.$row['fee_session'].'>>'.$dbname.'</h1>';
				//print_r($row);
				//die();
				...
				<td></td>
				<td><a href="edit_admission.php?id='.$row['sno'].'" target="_blank">Details</a></td>';
				$sql = 'select * from `'.$row['dbname'].'`.`fees_detail_misc` where student_id="'.$row['sno'].'"';
				$res_stu = execute_query(connect(), $sql);
				if(mysqli_num_rows($res_stu)!=0){
					echo '<td style="color:#0F0"><a href="deposit_misc_fees.php?id='.$row['sno'].'&dbn='.$row['dbname'].'" target="_blank">Deposit Fees</a></td>';
				}
				else{
					echo '<td><a href="deposit_misc_fees.php?id='.$row['sno'].'&dbn='.$row['dbname'].'" style="color:#F00" target="_blank">Allot & Deposit</td>';
				}
				echo '</tr>';
			}
			echo '<tr>
			...
			<div class="col-md-12">
				<div class="row">							
					<div class="col-md-4">							
						...
						<input type="date" name="dcr_date" class="form-control" value="<?php if(isset($_POST['dcr_date'])){echo $_POST['dcr_date'];}
						else{echo date("Y-m-d"); $_POST['dcr_date']=date("Y-m-d");} ?>">
					</div>
					<div class="col-md-4">							
						<label>Cash in Hand Opening :</label>
						<input type="text" class="form-control" name="cash_opening" id="cash_opening" onBlur="calculate_closing();">
					</div>
					<div class="col-md-4">							
						<label>Cash in Hand Closing :</label>
						<input type="text" class="form-control" name="cash_closing" id="cash_closing" readonly>
					</div>
				</div>
				<button type="button" name="get_opening_button" class="btn btn-primary" onClick="get_opening();" >GET OPENING</button>
			</div>						
		</div>
	</div>
	<?php } ?>
	<table width="100%" class="table table-striped table-hover rounded">
		...
		<tr>
			<td>1.</td>
			<td><input type="text" name="account_1" id="account_1" tabindex="<?php echo $tab++; ?>" onFocus="$('#current').val(1);" onBlur="tab_fill_bank(1, 4)"></td>
			<td><input type="text" name="amount_1" id="amount_1" onBlur="calculate_closing();" tabindex="<?php echo $tab++; ?>" onFocus="$('#current').val(1);" onBlur="tab_fill_bank(1, 4)"></td>
		</tr>
		<tr id="lastRow">
			<td colspan="3" align="center">
			<input type="submit" name="generate_dcr" value="Generate DCR" class="btn btn-success"">
...
<script>
function tab_fill_bank(id, tab){
	var current = $("#current").val();
	var id = $("#id").val();
	
	if(id==current){
		id++;
		var txt = '<tr><td>'+id+'.</td><td><input type="text" name="account_'+id+'" id="account_'+id+'" tabindex="'+(tab++)+'" onFocus="$(\'#current\').val('+id+');" onBlur="tab_fill_bank('+id+', '+(tab+4)+')"></td><td><input type="text" name="amount_'+id+'" id="amount_'+id+'" onBlur="calculate_closing();" tabindex="'+(tab++)+'" onFocus="$(\'#current\').val('+id+');" onBlur="tab_fill_bank('+id+', '+(tab+4)+')"></td></tr>';
		$(txt).insertBefore("#lastRow");
		$("#id").val(id);
	}	
}		
